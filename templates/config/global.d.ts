...
export const writeGlobalTypes = () => code`
/**
...
/** Storage — MinIO / S3-compatible or Google Cloud Storage object storage */
declare namespace Storage {
  ...
  interface GcsEnvConfig {
    projectId: string;
    bucket: string;
    /** Emulator override only (e.g. fake-gcs-server: "http://localhost:4443"). Omit for real GCS. */
    apiEndpoint?: string;
  }
...
declare namespace Seeder {
  type GeneratorKind = "default" | "faker" | "int" | "float" | "boolean" | "enum" | "fixed" | "skip" | "override";

  /** Field type the "Ignore Typing" override pretends the field has. */
  type OverrideType = "String" | "Int" | "BigInt" | "Float" | "Decimal" | "Boolean" | "DateTime" | "Json" | "Enum";
  ...
  /** Generators that can be nested inside an OverrideGenerator (no further nesting). */
  type BaseGenerator =
    | DefaultGenerator
    | FakerGenerator
    | IntGenerator
  ...
  interface OverrideGenerator {
    kind: "override";
    /** Field type to generate as, bypassing the field's real Prisma type. */
    overrideType: OverrideType;
    /** Manually entered pool, used when overrideType is "Enum". */
    enumValues?: string[];
    /** Generator applied under overrideType instead of the field's real type. */
    rule?: BaseGenerator;
  }

  type FieldGenerator = BaseGenerator | OverrideGenerator;
...
export default writeGlobalTypes;
