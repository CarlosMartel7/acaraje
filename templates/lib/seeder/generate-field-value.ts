...
const faker = imp("faker@@faker-js/faker")
const isRuleCompatibleWithType = imp("isRuleCompatibleWithType@@/lib/seeder/field-generators")

export const writeGenerateFieldValue = () => code`
...
function applyGeneratorRule(rule: Seeder.FieldGenerator, fieldName: string, fieldType: string, enumValues?: string[]): unknown {
  switch (rule.kind) {
    ...
    case "override": {
      const overrideEnumValues = rule.overrideType === "Enum" ? rule.enumValues : undefined;
      // Guard against a hand-edited config file nesting "override" inside itself (the type
      // system rules this out via BaseGenerator, but the config is untyped JSON on disk).
      const nestedKind: string | undefined = rule.rule?.kind;
      const nested = rule.rule && nestedKind !== "override" ? rule.rule : { kind: "default" as const };
      return applyGeneratorRule(nested, fieldName, rule.overrideType, overrideEnumValues);
    }
    case "default":
    default:
...
export default writeGenerateFieldValue;
